...
internal static class Entry
{
        ...
        OutputFormat format = OutputFormat.Text)
    {
        ...
        var res = await CommandHelpers.SendWithRetryOrWriteErrorAsync<IReadOnlyList<SymbolMatchGroup<SourceResult>>>(
        ...
        else
        {
            ...
            var groups = res.Result ?? Array.Empty<SymbolMatchGroup<SourceResult>>();
            for (var i = 0; i < groups.Count; i++)
            {
                TextOutput.WriteMatchHeader(groups[i].Symbol, groups[i].Kind);
                TextOutput.WriteSource(groups[i].Result, solutionPath);
                if (i < groups.Count - 1)
